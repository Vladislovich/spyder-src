...
void create_trackbar()
{
    ...
    cv::createTrackbar("Smoothing_for: i", "Controls", &Smoothing_for_int, 10, on_trackbar);
...
{
    //"{r:.4f} {theta:.4f}, {ard_ang:.4f}, {x:.4f}, {y:.4f}, {inten:.0f}
    std::string input_file = "../data/lidar_dataset_2.txt";
    ...
    while (!viewer.wasStopped()) { //q     
        if (treckbar_flag == 1) 
        {
            ...
            *cloud_intens = *cloud;
            ...
            for (int i = 0; i <= Smoothing_for_int; i++)
            {
                ...
                if (points_flag == flag_idx) //4
                {
                    ...
                    viewer.addPointCloud<pcl::PointXYZI>(cloud_intens, intensity_distribution, "cloud");
                    viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 2, "cloud");
                    std::cout << "SmoothByIntensity: " << i << std::endl;
                }

    ...
    while (!viewer.wasStopped()) { //q     
        if (treckbar_flag == 1) 
        {
            ...
            *cloud_intens = *cloud;
            ...
            cloud_normals = Smoothing(cloud_intens, flag_idx);
            if (points_flag == flag_idx) {
                view_normslize_cloud(cloud_normals, &viewer);
                std::cout << "Smoothing" << std::endl;
            }
            flag_idx++;

            //mesh = MeshingPoasson(cloud_normals, flag_idx);
            if (mesh_flag == 1) {  
                MeshingPoassonClaster2(cloud_normals, &viewer, flag_idx);
                /*mesh = Meshing(cloud_normals, flag_idx);              

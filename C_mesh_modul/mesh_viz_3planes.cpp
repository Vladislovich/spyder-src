...
int main()
{
    ...
    static int origin_point_viz = 0;
    global_treckbar.push_treckbar("origin_point_viz", &origin_point_viz, 5);
    static int origin_mesh_viz = 0;
    global_treckbar.push_treckbar("origin_mesh_viz", &origin_mesh_viz, 1);
    ...
    while (!viewer.wasStopped()) 
    {
        if (global_treckbar.treckbar_flag)
        {
            ...
            viewer.removePointCloud("origin_cloud");
            viewer.removePolygonMesh("origin_mesh");

            
            Mesh.OriginXYZI = Mesh.cloudXYZI_src;
            if (ref_point_viz == 0) Mesh.OriginXYZI_viz = Mesh.OriginXYZI;

            std::vector<float> origin_vector = {0.5, 0.86, 0.001, -1.00};
            Mesh.OriginXYZI = Mesh.generatePlane(origin_vector, 0.5);
            if (origin_point_viz == 1) Mesh.OriginXYZI_viz = Mesh.OriginXYZI;

            Mesh.OriginXYZIN = Mesh.computeNormals(Mesh.OriginXYZI);
            if (origin_point_viz == 2) Mesh.OriginXYZI_viz = Mesh.convertToXYZI(Mesh.OriginXYZIN);

            if (origin_point_viz <= 2)
            {
                pcl::visualization::PointCloudColorHandlerGenericField<pcl::PointXYZI> origin_distribution(Mesh.OriginXYZI_viz, "intensity");
                viewer.addPointCloud<pcl::PointXYZI>(Mesh.OriginXYZI_viz, origin_distribution, "origin_cloud");
                viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_POINT_SIZE, 3, "origin_cloud");
            }
            
            if (origin_mesh_viz == 1)
            {
                Mesh.origin_mesh = Mesh.Poisson_mesh(Mesh.OriginXYZIN);           
                viewer.addPolygonMesh(*Mesh.origin_mesh, "origin_mesh");
                viewer.setPointCloudRenderingProperties(pcl::visualization::PCL_VISUALIZER_COLOR, 1.0, 0.0, 0.0, "origin_mesh");
            }
        }

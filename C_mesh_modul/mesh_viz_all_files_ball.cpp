...
int main()
{
    ...
    std::string path = "../ball_data_filtered";
    for (const auto& entry : fs::directory_iterator(path))
    {
        ...
        while (key != 'q') 
        {
            if (global_treckbar.treckbar_flag)
            {
                ...
                if (ref_mesh_viz == 1)
                    viewer.addSphere(pcl::PointXYZ(coef[0], coef[1], coef[2]), coef[3], 1.0, 0.0, 0.0, "ransac_sphere");
                ...
                if (out.is_open() && write_to_file)
                    out << entry.path().filename() << ": " << Mesh.count_normal_error_ball(coef, Mesh.RefXYZI) << std::endl;
